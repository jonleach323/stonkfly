...
1. Create a new Solana keypair: `python -m stonkfly keygen` (or `solana-keygen new -o satrush-keypair.json` if you have the Solana CLI). It prints the public address only. Fund that address with **at most 100 USDC** (mainnet mint `EPjF...Dt1v`) and about 0.02 SOL for fees. Do not use a wallet holding anything else. With Docker: `docker compose run --rm worker keygen --out /runs/satrush-keypair.json`, then copy the file's contents into `SATRUSH_KEYPAIR_JSON` in `.env` (and delete the file from the volume if you like).
