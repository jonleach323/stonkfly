...
4. Some rounds are a **Sat Strike**: the strike vault (fed by the 2.08% strike fee) pays its bonus, in USDC and BTC, to the winning tile by the same pro-rata share (verified on round 56305: a $47 stake spread over all 21 tiles earned $368 and 163,748 sats of bonus). Paper settlement includes it when the round detail is tagged `is_sat_strike`.
5. Deploying also earns **epoch-vault tickets** (a 10-day iteration whose pool goes to ranked ticket holders), **1 BTC vault tickets** (a lottery) and **hashrate**. The API does not publish the ticket formula, so the worker reads a live wallet's tickets back from `/users/{wallet}/epoch` and `/users/{wallet}/one-btc` and shows them; paper runs have no wallet and no tickets.

Covering all 21 tiles is a hit every round but not a profit: on round 56751 an all-21 automation staked $50, got $42.38 of refunds and $4.41 of pot, and lost $2.56 (5.1%), the fee and haircuts less a share of the pot. Fewer tiles lose about the same on average with more variance. The readout picks tiles one at a time until no group is above its usual rate, so anything from one tile to all 21 can come out; the count is not a setting.
